...
static const char *polishMessages[] = {
	...
	"Uniksowe znaki końca linii",
	"DOSowe znaki końca linii",
	...
	"twój plik konfiguracyjny nie podaje pliku ciastek",
	...
	"nie podano pliku z czarną listą, funkcja wyłączona",
	"koniec wiadomości",
	...
	"nie można wczytać tekstu do sesji bazy danych",
	...
	"wiersz przekracza %d bajtów po podstawieniu",
	"brak wyrażenia regularnego po %c",
	...
	"%s nie jest URLem",
	"bezpieczne proxy nie zostało zaimplementowane",
	...
	"uwaga: pola bez nazw nie zostaną przesłane",
	"nie można się połączyć z %s",
	"nie można nawiązać bezpiecznego połączenia z %s, błąd %d",
	"Ceryfikat hosta %s nie mógł być zweryfikowany - połączenie SSL anulowane",
	"nie można wysłać zapytania do serwera",
	"metoda autoryzacji %s nierozpoznana",
	"strona wymaga autoryzacji",
	"logowanie anulowane",
	"nie można stworzyć pliku tymczasowego %s aby zdekompresować stronę",
	"nie można zapisać pliku tymczasowego %s aby zdekompresować stronę",
	"zcat nie może zdekompresować danych do rekonstrukcji strony",
	"nie można przejść do nieksompresowanej strony w %s",
	"nie można wczytać nieksompresowanej strony w %s",
	...
	"ftp: katalog nie istnieje",
	"ftp: nie można przejść do katalogu (przekroczono limit czasu)",
	"ftp: błędny URL",
	"ftp: błąd użycia",
	0,
	"ftp: inicjalizacja biblioteki zakończona niepowodzeniem",
	"ftp: inicjalizacja sesji zakończona niepowodzeniem",
	"ftp: nieoczekiwany bład %d",
	...
	"nie można wysłać danych do serwera pocztowego",
	"nie można wczytać danych z serwera pocztowego",
	"wiersz z serwera pocztowego jest zbyt długi lub niezakończony",
	"nie można zlokalizować serwera pocztowego %s",
	...
	"nieprawidłowe znaki w wierszu tematu, używaj tylko spacji i drukowalych znaków ASCII",
	...
	"nieoczekiwana odpowiedź '%s' na początku sesji sendmail",
	"serwer pocztowy nie rozpoznaje %s",
	"serwer pocztowy odrzucił %s",
	"serwer pocztowy nie jest gotów do odbioru danych, %s",
	...
	"hasło zbyt długie",
	"zbyt wiele pobrań z internetu, możesz chcieć wyłączyć opcję 'redirect html'",
	"strona pośrednio pobiera samą siebie, nieskończona pętla",
	"nie podano funkcji",
	...
	"plik jest zbyt duży, limit 40MB",
	...
	"uwaga: nierozpoznana metoda kompresji HTTP %s\n",
	"uwaga: przekierowanie HTTP %ld, ale brak nowego adresu URL\n",
	"uwaga: błąd HTTP %ld, %s\n",
	"uwaga: strona nie posiada rozpoznawalnego nagłówka HTTP",
	...
	".ebrc: czarna lista IP %s nie jest regularnym plikiem",
	...
	"zbyt wiele adresów IP na czarnej liście, limit %d",
	...
	"Nieoczekiwane wprowadzenie pop3 <%s>",
	"login / hasło nie jest dozwolona",
	"Nie można uzyskać informacji o statusie Twojej skrzynki odbiorczej <%s>",
	"błąd podczas pobierania wiadomości z serwera, błąd numer %d",
	"nie można pobrać wiadomości %d <%s>",
	"Przykro mi, minęło zbyt dużo czasu; serwer pocztowy zawiesił połączenie.",
	"nie można usunąć wiadomości <%s>",
	...
	"w obecnej chwili wspierana jest jedynie metoda 'auth login' Sendmail",
	"Nie można było zakończyć sekwencji SMTP login/hasło <%s>",
